...
module.exports = function extend(thorin, opt, Joi) {
  const storeObj = thorin.store(opt.store),
  ...
  Joi.modelId = (modelName, opt = {}) => {
    ...
    if (['INTEGER', 'BIGINT', 'FLOAT', 'DOUBLE', 'REAL', 'DECIMAL'].indexOf(fieldType) !== -1) {
      ...
      if (Field.type.options?.length) {
        if (Field.prefix) {
          joiCheck = joiCheck.min(Field.type.options.length - 6).max(Field.type.options.length); // allow growing from one size to another.
        } else {
